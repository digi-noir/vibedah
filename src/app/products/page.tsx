...
if (activeCategory) {
  ...
  if (query) {
    products = products.filter((p) =>
      `${p.brand} ${p.model} ${p.description} ${p.tags}`
        .toLowerCase()
        .includes(query),
    );
  }
    ...
    <div className="max-w-7xl mx-auto px-4 py-10">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
          ...
          <span className="text-gray-400 dark:text-slate-500 text-lg font-normal ml-3">
            ({products.length})
          </span>
        </h1>
      ...
      {/* Grid */}
      {products.length === 0 ? (
        <p className="text-gray-400 dark:text-slate-400 text-center py-20">
          No products found.
        </p>
      ) : (
